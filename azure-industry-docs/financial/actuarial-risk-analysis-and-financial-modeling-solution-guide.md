...
시스템을 최대한 활용 하려면 모델에 대 한 워크플로와 계산이 확장 하는 기능과 상호 작용 하는 방식을 이해 해야 합니다. 소프트웨어에는 작업, 작업 또는 이와 유사한 개념이 있을 수 있습니다. 이런 지식을 통해 작업을 분할할 수 있는 무언가를 설계합니다. 모델에 사용자 지정 단계가 있다면 처리를 위해 입력을 더 작은 그룹으로 분할할 수 있게 설계합니다. 종종 이를 분산-수집 패턴이라고 합니다.
